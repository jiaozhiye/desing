...
angular.module('myApp.upload.upload-directive', ['myApp.service.removedRecordService']).
    directive('appUpload', ['removedRecordService', function(removedRecordService){
        return {
            ...
            templateUrl: '/admin/src/components/upload/upload-template.html', // 模版
            controller: ['$scope', '$http', '$q', function UploadCtrl($scope, $http, $q){
                ...
                // 参数集合
                var params = {
                    flash_swf_url: '/admin/bower_components/plupload/js/Moxie.swf', // swf文件，当需要使用swf方式进行上传时需要配置该参数
                    silverlight_xap_url: '/admin/bower_components/plupload/js/Moxie.xap' // silverlight文件，当需要使用silverlight方式进行上传时需要配置该参数
                };
